...
SELECT * FROM orders WHERE ship_country = 'France' OR ship_country = 'Germany';
...
SELECT MIN(unit_price) discontinued, MAX(unit_price) discontinued FROM products WHERE discontinued != 1;
...
SELECT MIN(unit_price) discontinued, MAX(unit_price) discontinued FROM products WHERE discontinued != 1 AND units_in_stock > 20;
